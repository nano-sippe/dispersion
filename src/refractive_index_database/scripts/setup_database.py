#!/usr/bin/env python
"""
setup the refractive index database filestructure and configuration file
"""
import os
import tempfile
...
def get_root_dir(conf):
    ...
    """
    question = ("path for root directory of the database file" +
                " system [default: {}]> ")
    default = conf['Path']
...
def get_database_name(conf):
    """
    get the database file name from the user
    """
    question = ("name of the database file" +
                " [default: {}]> ")
    default = conf['File']
    validator = valid_file_name
    data_name = "database file name"
    return ask_and_confirm(question, default, validator, data_name)
...
def main():
    ...
    while not confirmed_db_nane:
        [name, confirmed_db_nane] = get_database_name(conf)
    conf['File'] = name
